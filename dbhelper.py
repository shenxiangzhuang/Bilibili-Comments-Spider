...
def get_all_comments(c):
    sql = "SELECT floor, mid, username, rpid, gender,\
...
def insert_comment(c, conn, comment):
    sql = "INSERT INTO " + comment_tablename + \
        """(mid, username, rpid, gender, content, ctime, likes, rcount)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"""
    c.execute(sql, (comment.mid, comment.username, comment.rpid,
                    comment.gender, comment.content, comment.ctime,
                    comment.likes, comment.rcount))
    conn.commit()
